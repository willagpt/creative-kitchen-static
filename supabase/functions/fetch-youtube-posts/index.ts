...
//     limit?: number = 20,            // videos per fetch (max 50)
//     mode?: "fetch" | "test" = "fetch",
//     quota_budget?: number = 10000 } // monthly quota soft cap
//
// Quota model (YouTube Data API v3):
//   playlistItems.list (contentDetails) = 1 unit
//   videos.list (snippet, contentDetails, statistics) = 1 unit / call
//   Per fetch: ~2 quota units for limit<=50.
//
// Shorts detection:
...
const FUNCTION_VERSION = "fetch-youtube-posts@1.0.0";
...
async function listUploads(
  ...
  params.append("maxResults", String(Math.min(50, limit)));
  ...
  for (const it of data.items || []) {
    ...
    if (vid) {
      items.push({ videoId: vid, publishedAt: it.contentDetails?.videoPublishedAt || it.snippet?.publishedAt });
    }
  }
  return items.slice(0, limit);
}

async function videosList(videoIds: string[]): Promise<YtVideoResource[]> {
  if (!YT_API_KEY) throw new Error("YOUTUBE_API_KEY secret not set");
  ...
  const data = await res.json();
  return (data.items || []) as YtVideoResource[];
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 2. quota guard (projected = 2 units)
    const projectedQuota = 2;
    ...
    // 4. list uploads
    const uploads = await listUploads(account.uploads_playlist_id, limit);
    const videoIds = uploads.map(u => u.videoId);
    if (videoIds.length === 0) {
      if (logId) {
        await closeLog(logId, { posts_fetched: 0, posts_new: 0, yt_quota_units: 1, status: "success" });
      }
      return jsonResponse({
        success: true, mode, account: { id: account.id, handle: account.handle, brand_name: account.brand_name },
        posts_fetched: 0, posts_upserted: 0, posts_new: 0, quota_units_used: 1,
      });
    }

    // 5. fetch full video details
    const videos = await videosList(videoIds);
    ...
    const quotaUsed = 2; // 1 for playlistItems + 1 for videos.list
