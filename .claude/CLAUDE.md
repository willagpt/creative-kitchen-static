...
- **Next:** Phase 3b — Supabase RPC joining `followed_organic_accounts` to latest `organic_fetch_log` + per-account post counts (replace client-side grouping). Phase 3c — scheduled cron (pg_cron or Supabase Scheduled Functions) for daily IG + 2x daily YT fetches with staggered kickoffs. Video Analysis Engine Phase 2 (Whisper transcription + OCR).
