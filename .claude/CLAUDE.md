...
- **Organic Intelligence Phase 3c complete (17 Apr):** Scheduled ingestion shipped end-to-end, cron DISABLED on ship pending key rotation + vault populate. New orchestrator edge function `trigger-organic-fetches` (v2) with per-platform budget guards (IG $1.00/UTC-day, YT 8000 units/UTC-month), due-account filter on `last_fetched_at`, and staggered dispatch to `fetch-instagram-posts` / `fetch-youtube-posts`. Migration `20260417140000_phase3c_cron_schedule.sql` installs pg_cron, adds SECURITY DEFINER helper `public._trigger_organic_platform(text)` that reads the service role JWT from `vault.decrypted_secrets` (secret name `organic_cron_service_key`) and POSTs via `net.http_post`, and schedules three jobs: `organic_fetch_instagram_daily` at 02:15 UTC, `organic_fetch_youtube_morning` at 06:30 UTC, `organic_fetch_youtube_evening` at 18:30 UTC, all `active = false` on ship. Observability RPC `public.list_fetch_runs_summary(p_since)` rolls up `organic_fetch_log` into daily per-platform rows (runs, status breakdown, posts_fetched/new, cost_estimate, yt_quota_units). UI strip `.oi-runs-strip` on Organic Intel tab renders a "Last 7 days" chip per platform from that RPC. Verified via two dry_runs (IG budget_used $0.046 of $1.00, YT 4 of 8000; zero `organic_fetch_log` rows created). See `docs/handover-2026-04-17-organic-intel-phase-3c.md`.
- **Next:** Rotate Apify + YouTube API keys (both leaked earlier in session), then `select vault.create_secret('<SERVICE_ROLE_JWT>', 'organic_cron_service_key', '...')` and `select cron.alter_job(jobid, active := true) from cron.job where jobname like 'organic_fetch_%';` to arm the schedule. Video Analysis Engine Phase 2 (Whisper transcription + OCR).
