    ...
    return (
      <div key={ad.adId + '-' + (ad.cardIndex || '')} className={`ca-card ${isAnalysing ? 'ca-card-analysing' : ''}`}>
        <div className="ca-card-media">
          ...
          {/* When analysed, button doubles as 'Playbook' opening /single-ad-playbook.html */}
          {showBrandTag && ad.isVideo && ad.hasMedia && (
            <button
              ...
              onClick={(e) => {
                ...
                if (isAlreadyAnalysed) {
                  window.open(`/single-ad-playbook.html?ad_id=${ad.adId}`, '_blank', 'noopener,noreferrer');
                } else {
                  handleAnalyseVideo(ad);
                }
              }}
              disabled={isAnalysing}
              title={isAlreadyAnalysed ? 'Open creative playbook for this ad' : isAnalysing ? 'Analysing...' : 'Analyse this video with AI'}
            >
              {isAnalysing ? (
                <><span className="ca-spin-sm"></span> Analysing</>
              ) : isAlreadyAnalysed ? (
                <><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg> Playbook</>
              ) : (
            ...
            <>
              ...
              {!topLoading && topFiltered.length > 0 && (
                <>
                  ...
                  {(() => {
                    const eligibleVideos = topFiltered.filter(a => a.isVideo && a.hasMedia && !analysedAdIds.has(a.adId))
                    const hasEligible = eligibleVideos.length > 0
                    return (
                      <div className="ca-bulk-toolbar" style={{ display: (selectedVideoIds.size > 0 || hasEligible) ? 'flex' : 'none' }}>
                        {hasEligible && selectedVideoIds.size === 0 && (
                          <button className="ca-bulk-select-btn" onClick={() => {
                            const ids = new Set(eligibleVideos.slice(0, 20).map(a => a.adId))
                            setSelectedVideoIds(ids)
                          }}>
                            Select All Videos ({Math.min(eligibleVideos.length, 20)})
                          </button>
                        )}
